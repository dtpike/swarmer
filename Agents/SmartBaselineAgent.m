classdef SmartBaselineAgent < Agent
    %SMARTBASELINEAGENT Subclass of Agent - Follows calculated tour using Rathinam method 
    % to split into tours for each vehicle. Uses optimal TSP instead of Eulerian tour. See RathinamAgent
    %Same code as BaselineAgent, but triggers a call in CalculatePaths to
    %optimize the direction of the tour (minimizing average service time)
    %Also, if no more targets in tour, moves to closest target.
    %
    ...
    methods
        function obj = SmartBaselineAgent()
            %constructor
            obj = obj@Agent();
        end
